...
export const FormComponent = () => {
  ...
  const calculatePaymentDates = (numParcelas: number) => {
    const dates = [];
  ...
  const generatePDF = () => {
    ...
    autoTable(doc, {
      head: [["Marca/Modelo", "Placa", "Serviço", "Valor", "Gorjeta", "Pago", "Método de Pagamento", "Parcelas", "Datas de Pagamento",]],
      body: lista.map((item) => [
        item.marca,
        item.placa,
        item.servico,
        ...
        item.metodoPagamento === "CartãoCredito" && item.parcelas > 1
          ? calculatePaymentDates(item.parcelas).join(", ")
          : "N/A",
      ]),
      startY: 40,
      styles: { fontSize: 7 },
          ...
          {lista.map((item, index) => (
            ...
            >
              <div className="flex flex-row">
                <p className="flex flex-col font-bold mr-4">
                  Marca:
              ...
              <div className="flex flex-row mt-2">
                <p className="flex flex-col font-bold mr-4">
                  Valor:
                  <span className="font-semibold"> R$ {item.pagamento.toFixed(2)}</span>
                </p>
                <p className="flex flex-col font-bold mr-4">
                  Método de Pagamento:
                ...
                <div className="flex flex-row">
                  <label className="font-bold text-[#cd3200] mr-2">Gorjeta:</label>
                  <input
                    type="text"
                    value={item.gorjeta}
                    onChange={(e) =>
              ...
              <div className="flex flex-row mt-2">
                <label className="font-bold text-[#cd3200] mr-2">Pago</label>
                <input
                  type="checkbox"
